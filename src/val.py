...
from utils import convert, log, visualisation
from utils import postproc
from depth_trainer import normalize_rgb
...
def get_masks_davis(cfg, masks_softmaxed, sample_dict, force_ccpp=False, k=None):
    tspec = {
        "dtype": masks_softmaxed.dtype,
        "device": masks_softmaxed.device,
    }
    ...
    if k is None:
        k = masks_softmaxed_sel.shape[1]
        if masks_softmaxed_sel.shape[0] == 1:
            k = convert.to_5dim_hard_mask(sample_dict["sem_seg_ori"], **tspec).shape[1]

    true_masks = convert.to_5dim_hard_mask(sample_dict["sem_seg_ori"], k, **tspec)
    pred_masks = convert.to_5dim_hard_mask(masks_softmaxed_sel, k, **tspec)
...
def run_evaluation(
    cfg, val_loader, model, depth_model, writer=None, writer_iteration=0, video=False, prefix="eval"
):
    logger.info(
    ...
    with torch.no_grad():
        ...
        for idx, sample in enumerate(tqdm(val_loader, disable=False)):
            t = len(sample[0])
            ...
            disp = F.interpolate(depth_model(rgb).unsqueeze(1),  preds_dict['sem_seg'].shape[-2:])    # Bx1xHxW            
            ...
            if writer is not None:
                ...
                visualizer.add(
                    sample_dict, disp, preds_dict, masks_softmaxed, pred_masks, true_masks
                )
        ...
        return {'mIoU': mIoU, 'mIoU_fg': mIoU_fg, 'ARI': ARI, 'ARI-F': ARI_FG, 'acc': ACC, 'sdif': SDIF,
                'mIoU_PP': pp_evaluator.statistic("mIoU"), 'ARI_PP': pp_evaluator.statistic("ARI"), 'ARI-F_PP': pp_evaluator.statistic("ARI_FG")}
