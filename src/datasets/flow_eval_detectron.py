...
class FlowEvalDetectron(Dataset):
    def __init__(self, data_dir, resolution, pair_list, val_seq, to_rgb=False, with_rgb=False, size_divisibility=None,
                 small_val=0, flow_clip=1., norm=True, read_big=False, eval_size=True, force1080p=False):
        ...
        for v in self.val_seq:
            seq_dir = Path(self.data_dir[1]) / v
            frames_paths = sorted(seq_dir.glob('*.jpg'))
    ...
    def __getitem__(self, idx):
        ...
        dataset_dict = {}
        flow_dir = Path(self.data_dir[1]) / self.samples[idx]
        ...
        rgb = rgb.clip(0., 255.)
        d2_utils.check_image_size(dataset_dict, rgb.transpose(1,2,0))
        ...
        dataset_dict["flow"] = torch.zeros(2, rgb.shape[-2], rgb.shape[-1])
